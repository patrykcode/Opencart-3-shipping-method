...
class ModelExtensionShippingInpost extends Model {
	public function getQuote($address) {
		...
		foreach ($query->rows as $result) {
			if ($this->config->get('shipping_inpost_' . $result['geo_zone_id'] . '_status')) {
			...
			if ($status) {
				...
				$rates = explode(',', $this->config->get('shipping_inpost_' . $result['geo_zone_id'] . '_rate'));
				...
				if ((string)$cost != '') {
					$quote_data['inpost_' . $result['geo_zone_id']] = array(
						'code'         => 'inpost.inpost_' . $result['geo_zone_id'],
						'title'        => $result['name'] . '  (' . $this->language->get('text_inpost') . ' ' . $this->weight->format($weight, $this->config->get('config_inpost_class_id')) . ')',
						'cost'         => $cost,
