...
class ControllerExtensionShippingInpost extends Controller {
	...
	public function index() {
		...
		foreach ($geo_zones as $geo_zone) {
			if (isset($this->request->post['shipping_inpost_' . $geo_zone['geo_zone_id'] . '_rate'])) {
				$data['shipping_inpost_geo_zone_rate'][$geo_zone['geo_zone_id']] = $this->request->post['shipping_inpost_' . $geo_zone['geo_zone_id'] . '_rate'];
			} else {
				$data['shipping_inpost_geo_zone_rate'][$geo_zone['geo_zone_id']] = $this->config->get('shipping_inpost_' . $geo_zone['geo_zone_id'] . '_rate');
			}

			if (isset($this->request->post['shipping_inpost_' . $geo_zone['geo_zone_id'] . '_status'])) {
				$data['shipping_inpost_geo_zone_status'][$geo_zone['geo_zone_id']] = $this->request->post['shipping_inpost_' . $geo_zone['geo_zone_id'] . '_status'];
			} else {
				$data['shipping_inpost_geo_zone_status'][$geo_zone['geo_zone_id']] = $this->config->get('shipping_inpost_' . $geo_zone['geo_zone_id'] . '_status');
			}
		}

		$data['geo_zones'] = $geo_zones;

		if (isset($this->request->post['shipping_inpost_tax_class_id'])) {
			$data['shipping_inpost_tax_class_id'] = $this->request->post['shipping_inpost_tax_class_id'];
		} else {
			$data['shipping_inpost_tax_class_id'] = $this->config->get('shipping_inpost_tax_class_id');
		}
